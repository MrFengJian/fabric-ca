...
func signCert(req signer.SignRequest, ca *CA) (cert []byte, err error) {
	...
	if err != nil {
		log.Errorf("ParseCertificateRequest error:[%s]", err)
		return nil, err
	}

	certfile := ca.Config.CA.Certfile
	//certfile := req.Profile
	log.Debugf("certifle = %s", certfile)
	rootkey, _, x509cert, err := util.GetSignerFromCertFile(certfile, ca.csp)
	if err != nil {
		log.Errorf("failed to get signer from cert file %v", err)
		return nil, err
	}
	log.Debugf("x509cert = %v", x509cert)
	...
	if err != nil {
		log.Errorf("failed to read sm2 certificate %v", err)
		return nil, err
	}
	...
	serial := util.GetSerialAsHex(clientCert.SerialNumber)
	...
	if err != nil {
		log.Errorf("failed to insert certificate record", err)
	}
	return
}
...
func createGmSm2Cert(key bccsp.Key, req *csr.CertificateRequest, priv crypto.Signer) (cert []byte, err error) {
	log.Debugf("try to create sm2 cert:%T", key)

	csrPEM, err := generate(priv, req, key)
	if err != nil {
		log.Errorf("create csr error:%v", err)
		return nil, err
	}
	block, _ := pem.Decode(csrPEM)
	if block == nil {
		log.Errorf("failed to decode sm2 csr:%v", err)
	...
	if err != nil {
		log.Errorf("parseCertificateRequest return err:%v", err)
		return nil, err
	}
	log.Debugf("got sm2 template %T %T", sm2Template.PublicKey, sm2Template)
	//TODO: keyusage，extkeyusage的集成使用
...
func parseCertificateRequest(csrBytes []byte) (template *sm2.Certificate, err error) {
	...
	if err != nil {
		log.Errorf("failed to parse sm2 cert request", err)
	...
	log.Debugf("public key algorithm[%v], signature algorithm[%v]", template.PublicKeyAlgorithm, template.SignatureAlgorithm)
	...
	//TODO: keyusage，extkeyusage的集成使用
	// 默认可以usage 为cert usage和crl usage []string{"cert sign", "crl sign"}
...
func generate(priv crypto.Signer, req *csr.CertificateRequest, key bccsp.Key) (csr []byte, err error) {
	...
	log.Debug("create sm2 certificate request", err)
	return csr, err
}
