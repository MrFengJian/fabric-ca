...
func VerifyToken(csp bccsp.BCCSP, token string, method, uri string, body []byte, compMode1_3 bool) (*x509.Certificate, error) {
	...
	sm2Cert := gm.ParseX509Certificate2Sm2(x509Cert)
...
func GetX509CertificateFromPEM(cert []byte) (*x509.Certificate, error) {
	...
	// TODO: 有更好的办法判断证书类型之前，先按sm处理
	var x509Cert *x509.Certificate
	var err error
	if sm2Cert, err := sm2.ParseCertificate(block.Bytes); err != nil {
		log.Errorf("failed to parse cert to sm2,may be is x509:%v", err)
		x509Cert, err = x509.ParseCertificate(block.Bytes)
	} else {
		x509Cert = ParseSm2Certificate2X509(sm2Cert)
		log.Infof("sm2 cert it %#v", sm2Cert.KeyUsage)
		log.Infof("x509 cert %#v", x509Cert.KeyUsage)
	}
	if err != nil {
